...
@interface XLImageViewer ()<UICollectionViewDelegate,UICollectionViewDataSource>
{
    ...
    NSArray *_currentSelectImageArr;//传过来的原始图片数组
    
    ImagesBlock _selectBlock;
}
//滚动的ScrollView
...
- (void)buildUI{
    ...
    [_toolBar addSelectBlock:^(NSArray *selectImageArr) {
        [weekSelf selectImageArr:selectImageArr];
    }];
...
-(void)selectFinishedBlock:(ImagesBlock)selectBlock{
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    } finish:^(NSArray *selectImageArr) {
        self->_selectBlock(selectImageArr);
...
-(void)showNetImages:(NSArray <NSString *>*)imageUrls index:(NSInteger)index fromImageContainer:(UIView*)imageContainer currentSelectImageArr:(NSArray *)arr{
    
    _showNetImages = true;
    
    _currentSelectImageArr = arr;
...
//    _anchorFrame = [_imageContainer convertRect:_imageContainer.bounds toView:self];
    ...
    //更新显示
    _toolBar.currentSelectImageArr = _currentSelectImageArr;
    _toolBar.text = [NSString stringWithFormat:@"%zd/%zd",_currentIndex + 1,_imageUrls.count];
    [_toolBar show];
...
#pragma mark 选择图片方法
- (void)selectImageArr:(NSArray *)selectImageArr {
    XLImageViewerItem *item = (XLImageViewerItem *)[_collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:_currentIndex inSection:0]];
    [item selectImageArr:selectImageArr];
}
